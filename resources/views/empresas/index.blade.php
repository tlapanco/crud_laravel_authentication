...
<div class="container-fluid p-0">
    ...
    <div class="col-12 bg-primario rounded-top mb-3">
        <div class="col-12 py-4">
            <h1 class="text-white fw-bold text-uppercase text-center fs-4 mb-2">Control de empresas</h1>
        </div>
    </div>
    <!-- Container Slogan -->
    ...
    <div class="row w-100 h-100 m-0 p-0">
        ...
        <!-- Header Section -->
        <form action="{{ route('empresas.index') }}" method="POST" class="row w-100 p-0 mx-auto align-items-center" id="form-number-items">
            ...
            <div class="col-12 col-md-4">
                <a href="{{ route('empresas.create') }}" class="btn btn-primario text-white btn-sm d-block mx-auto w-75">Crear Nuevo</a>
            </div>
        </form>
        <!-- Header Section -->
                ...
                <table class="table table-primaria d-none d-lg-table text-center align-center">
                    ...
                    <tbody id="">
                        ...
                        <tr class="d-none d-lg-table-row col-12">
                            <td class="col-1">{{ $item->rfc}}</td>
                            <td class="col-1">{{ $item->nombre}}</td>
                            <td class="col-1">{{ $item->direccion}}</td>
                            <td class="col-1">{{ $item->ti}}</td>
                            <td class="col-1">{{ $item->tam}}</td>
                            <td class="col-3">
                                <span
                                    @class([
                              ...
                              {{--  start botón de editar --}}
                                <div class="row justify-content-evenly w-100">
                                    <div class="col text-center px-0">
                                        <a href="{{ route('empresas.edit', $item) }}" class="btn btn-sm btn-primario text-white w-20" >
                                    ...
                                    <div class="col text-center px-0">
                                        <form action="{{ route('empresas.toggle.status', $item) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                    ...
                                    <div class="col text-center px-0">
                                        @if(!$item->activo)
                                        <form action="{{ route('empresas.destroy', $item) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col text-center">
                        <a href="{{ route('empresas.edit', $item) }}" class="btn btn-sm btn-primario text-white">Editar</a>
                    </div>
                    <div class="col text-center">
                        <form action="{{ route('empresas.toggle.status', $item) }}" method="POST">
                            @csrf
