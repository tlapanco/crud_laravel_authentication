...
class ReporteController extends Controller
{
    ...
    public function consulta()
    {
        ...
        return view('reportes.solicitudes_pendientes.consulta')
            ->with(['ciclos' => $ciclos])
    ...
    public function contenido(Request $request)
    {
        ...
        $listas = DB::select("
            ...
            WHERE (carta_subido='NO' OR solicitud_estatus='PENDIENTE') AND grupos.idce = $idce AND solicitudes.idp = $idp
            GROUP BY solicitudes.idg,carreras.nombre,carreras.idca) AS t1
            GROUP BY t1.nombre,t1.idca;
        ");

        return view("reportes.solicitudes_pendientes.contenido")
            ->with(['$ciclos' => $ciclos])
            ->with(['procesos' =>$procesos])
            ->with(['idce' => $idce])
            ->with(['idp' => $idp])
    ...
    public function detalle_carreras($id, $idce, $idp)
    {
        //idce del ciclo escolar seleccionado
        $idce = $idce;
        ...
        $datos = DB::select("SELECT distinct
            ca.idca as idca, ce.idce as idce,
        ...
        $listas = DB::select(
            "SELECT g.nombre AS nombre, g.idg AS idg, COUNT(*) as nalu, 
            sum(estatus.solicitud_estatus='PENDIENTE') as solis, sum(estatus.carta_subido='NO') as cartas
        FROM estatus
        INNER JOIN solicitudes as ac ON ac.idpe = estatus.idpe
        INNER JOIN grupos AS g ON g.idg = ac.idg
        WHERE g.idca = $idca AND g.idce = $idce AND  (carta_subido='NO' OR solicitud_estatus='PENDIENTE') AND ac.idp = $idp
        GROUP BY g.nombre, g.idg");
        return view("reportes.solicitudes_pendientes.carrera")
            ->with(['idce' => $idce])
            ->with(['idca' => $idca])
            ->with(['datos' => $datos[0]])
            ->with(['listas' => $listas])
    ...
    //-------------------------------start detalle_grupo---------------------------------------------
    public function detalle_solicitudes($id, $idp, $idce, $idca, Request $request)
    {
       ...
       $usuarios = DB::select(
            "SELECT ac.nombre AS nombreu, ac.matricula as matricula, ac.correo as email
        FROM estatus
        INNER JOIN solicitudes as ac ON ac.idpe = estatus.idpe
        INNER JOIN grupos AS g ON g.idg = ac.idg
        WHERE g.idca = $idca AND g.idce = $idce AND  solicitud_estatus='PENDIENTE' AND ac.idp = $idp
        and ac.idg=$idg && ac.nombre LIKE '%$request->q%' ");

        return view("reportes.solicitudes_pendientes.grupos_solicitudes")
            ->with(['idg' => $idg])
            ->with(['datos' => $datos[0]])
            ->with(['usuarios' => $usuarios])
            ->with(['idp' => $idp]);
    }
    public function detalle_cartas($id, $idp, $idce, $idca, Request $request)
    {
        //idg del grupo seleccionado
        $idg = $id;
        $idp = $idp;
       ...
       $usuarios = DB::select(
            "SELECT ac.nombre AS nombreu, ac.matricula as matricula, ac.correo as email, empresas.nombre as empresa
        FROM estatus
        INNER JOIN solicitudes as ac ON ac.idpe = estatus.idpe
        INNER JOIN empresas ON empresas.idem = ac.idem
        INNER JOIN grupos AS g ON g.idg = ac.idg
        WHERE g.idca = $idca AND g.idce = $idce AND  carta_subido='NO' AND ac.idp = $idp
        and ac.idg=$idg && ac.nombre LIKE '%$request->q%' ");

        return view("reportes.solicitudes_pendientes.grupos_cartas")
            ->with(['idg' => $idg])
            ->with(['datos' => $datos[0]])
            ->with(['usuarios' => $usuarios])
            ->with(['idp' => $idp]);
    ...
    //-------------------------------start regresar---------------------------------------------
    public function regresar($idce, $idp)
    {
        //Consulta todos los ciclos escolares
        $ciclos = DB::select('SELECT * FROM ciclo_escolares ORDER BY nombre ASC');
        $procesos = DB::select('SELECT * FROM procesos ORDER BY nombre ASC');
